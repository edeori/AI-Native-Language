...
- **Review**: versioned review outputs, with prompts generated by the MCP server
...
4. The importer also writes `source.semantic.json` as the structured learning snapshot for downstream graph and validation steps, plus `source.codegraph.json` / `source.codegraph.md` for the code-first knowledge graph derived from the remote Java parser MCP / AST pipeline, `source.recon.json` / `source.recon.prompt.md` for the deeper reconnaissance dossier, and `source.database.json` / `source.database.md` for the discovered persistence model. The final review prompts are generated by the MCP server at runtime so prompt changes do not require a plugin rebuild.
